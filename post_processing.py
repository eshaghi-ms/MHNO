...
def plot_field_trajectory(domain, fields, field_names, time_steps, plot_range, problem, plot_show=True, interpolation=True, colorbar=True):
    # colors = ["white", "black", "yellow"] if fields[0].ndim == 3 else ["white", "blue"]  # For AC and CH
    colors = ["white", "blue", "yellow"] if fields[0].ndim == 3 else ["white", "blue"]  # For SH
    custom_cmap = LinearSegmentedColormap.from_list("two_phase", colors, N=100)
    folder = problem + "/plots/fields/"
    os.makedirs(folder, exist_ok=True)
    ...
    for time_step in time_steps:
        ...
        for field, field_name, domain_range in zip(fields, field_names, plot_range):
            shot = field[..., time_step].cpu().numpy()
            if shot.ndim == 3:
                Nx = shot.shape[0]
                Ny = shot.shape[1]
                Nz = shot.shape[2]
                ...
                #            vmin=domain_range[0], vmax=domain_range[1], aspect='equal', interpolation=interpolation_opt)  # For AC, CH and SH
                # plt.imshow(shot, extent=(domain[0], domain[1], domain[0], domain[1]), origin='lower', cmap=custom_cmap,
                ...
                #     v_max = v_max * 0.25
                #
                # plt.imshow(shot, extent=(domain[0], domain[1], domain[0], domain[1]), aspect='equal', cmap='turbo',
